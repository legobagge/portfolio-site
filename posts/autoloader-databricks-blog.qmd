...
Let me tell you about a problem I ran into recently. We had daily JSON files dropping into Azure Blob Storage from an Oracle database—full dumps arriving every morning at 2 AM. Simple enough, right? Just read the files with `spark.read.json()` and load them into a Delta table.
...
This wasn't just inefficient—it was creating duplicate data, wasting compute resources, and making my data pipeline unnecessarily slow and expensive.
...
Each file is a **complete dump** of the orders table—about 25MB per file. New file arrives every morning at 2 AM.

### The Old Way (spark.read)

``` python
# Read all files every single time 😱
...
-   Day 1: Reads 1 file ✅
-   Day 2: Reads 2 files (1 duplicate) ❌
-   Day 30: Reads 30 files (29 duplicates) ❌❌❌
...
### Step 1: Autoloader → Bronze (Append Everything)

Bronze is your "raw data lake"—just land the data as-is, with minimal processing. Autoloader is perfect for this.
...
df_stream = (
    spark.readStream
    .format("cloudFiles")  # This is the magic ✨
...
Autoloader maintains a checkpoint—a hidden ledger of every file it's processed.

```         
Day 1: Process orders-2025-11-26.json → Checkpoint: ✓
Day 2: Check checkpoint → Skip orders-2025-11-26.json, 
       process orders-2025-11-27.json → Checkpoint: ✓✓
Day 3: Check checkpoint → Skip both old files, 
       process orders-2025-11-28.json → Checkpoint: ✓✓✓
```
...
I'm extracting the date from the filename (`orders-2025-11-26.json` → `2025-11-26`). This tells me **when the Oracle snapshot was taken**, not when I ingested it.
...
Think of Bronze as your "receipts drawer"—you keep everything, even if some orders appear in multiple daily dumps. This gives you:
...
-   **Debugging**: "What did the source system send us on Dec 5th?" → Query Bronze with `_file_date = '2025-12-05'`
...
```         
Day 1:  Read 1 file  (25 MB)  → 25 MB total
Day 2:  Read 2 files (50 MB)  → 75 MB total
Day 30: Read 30 files (750 MB) → 11,625 MB total (11.6 GB)
```
...
```         
Day 1:  Read 1 file  (25 MB)  → 25 MB total
Day 2:  Read 1 file  (25 MB)  → 50 MB total
Day 30: Read 1 file  (25 MB)  → 750 MB total
```
...
For my use case (daily batch), `availableNow=True` is ideal—it runs, finishes, and shuts down the cluster to save costs.
...
In my next post, I'll cover the **Silver layer**—how to deduplicate these daily full dumps and create a clean, analytics-ready table using the `_file_date` we extracted. We'll use window functions to keep only the latest version of each order and add proper data types.

But for now, if you're still manually tracking which files you've processed, give Autoloader a try. It's one of those tools that feels like magic the first time it works—and then you wonder how you ever lived without it.
